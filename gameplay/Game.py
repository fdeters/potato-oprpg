...
class Game:
    ...
    def get_command(self):
        ...
        command = input(
            "Enter your command | [E] to roll a new event | [H] to hurl potatoes at orcs: "
        )
        try:
            command = command[0]
        except IndexError:
    ...
    def handle_event_outcomes(self, event_outcomes: list) -> None:
        for outcome in event_outcomes:
            ...
            try:
                ...
                message = (
                    f"{stat.capitalize()} score went {up_or_down} by {abs(magnitude)}"
                )
            finally:
    ...
    def check_for_game_over(self) -> None:
        condition = (
            self._player.destiny >= 10
            or self._player.potatoes >= 10
            or self._player.orcs >= 10
        )
